...
Class Content extends DBConnection {
	...
	public function project() {
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(!in_array($k,array('id','name', 'category', 'summary', 'attachment_type', 'attachment_link', 'year'))){
				if(!empty($data)) $data .= ", ";
				$data .= "`$k` = '$v'";
			}
		}
		...
		$data .= "`attachment_type` = '".addslashes($attachment_type)."', ";
		$data .= "`attachment_link` = '".addslashes($attachment_link)."', ";
		$data .= "`year` = '".addslashes($year)."'";
